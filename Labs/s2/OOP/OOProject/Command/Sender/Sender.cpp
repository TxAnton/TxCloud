...
void Sender::set(std::unique_ptr<AbstractCommand> command) {
    ...
    switch (cmd->commandType) {
        ...
        case CommandType::CommandSelect: {
            ...
            logger << ("Player " + n + " selected " + (nCmd->AB ? "A" : "B") + " " + std::to_string(nCmd->x) + ":" +
                       std::to_string(nCmd->y));
            break;
        }
        ...
        case CommandType::CommandSummon: {
            ...
            logger << ("Player " + n + " " + "summoned unit class " + std::to_string(int(nCmd->unitClass)) +
                       " at " + std::to_string(nCmd->x) + ":" + std::to_string(nCmd->y));
            break;
        }
    }
