...
void Game::run() {
    ...
    mediator->createAt(UnitClass::ALPHA, UnitDevotion::LIGHT, 0, 0);

    dynamic_cast<Alpha *>(mediator->getObjAt(0, 0))->setUnitLim(field->getSize().first * field->getSize().second);


    mediator->createAt(UnitClass::ALPHA, UnitDevotion::DARK, 5, 5);
    ...
    CommandStep commandStep(mediator,facade,false,0,0,5,5);

    facade->pushCommand(std::dynamic_pointer_cast<Command>( std::shared_ptr<CommandStep>( &commandStep)));
