...
static BMPImage* BMPRotate(BMPImage* image, int32_t x1, int32_t y1, int32_t x2, int32_t y2, int angle);
...
BMPImage* BMPRotate(BMPImage* image, int32_t x1, int32_t y1, int32_t x2, int32_t y2, int angle){
    int32_t x_min = _min(x1, x2);
    int32_t x_max = _max(x1, x2);
    int32_t y_min = _min(y1, y2);
    int32_t y_max = _max(y1, y2);
    ...
    //(x_max-x)
    //(y-y_min)
    BMPImage* _img = BMPCut(image,0,0,BMPWidth(image),BMPHeight(image));
#define I (i-x_max)
#define J (j-y_min)
    double c = cos(_angle);
    double s = sin(_angle);
    for (int i = 0; i < BMPWidth(image); i++) {
        for (int j = 0; j < BMPHeight(image); j++) {
            //if((i*c+y*s) )
            int xx = (I*c+J*s)+x_max;
            int yy = (-I*s+J*c)+y_min;
            if(_inBounds(xx,x_min,x_max) && _inBounds(yy,y_min,y_max)){
                BMPSetPixel(_img,i,j,BMPGetPixel(image,xx,yy));
            }
        }
    }
    return _img;
