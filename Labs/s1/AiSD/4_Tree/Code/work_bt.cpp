...
int parseArgs(int argc, char **argv, string &iFileName,
              string &oFileName);    //Parses arguments. Returns 1 if program is to be closed
...
int main(int argc, char **argv) {
    ...
    do {///Main inpit-process-output-out cycle
        ...
        printStr("Counting deepest level(max depth of l/r branches+1) = \n");
        tmp = hBT(b);
        printStr("Deepest level = ");
        printInt(tmp);
        printStr("\n");

        printStr("Counting total tree lenght(sum of l/r brances + self's)\n");
        tmp = mBT(b);
