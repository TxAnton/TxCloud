...
//........................
    ...
    {
        int depth;
        if(isNull(s)||isAtom(s)){return 0;}
        else
        {
            lisp l = s;

            int _maxDepth=maxDepth(l->node.pair.hd);
            while(!isNull(l->node.pair.tl))
            {
                l=l->node.pair.tl;
                depth=maxDepth(l->node.pair.hd);
